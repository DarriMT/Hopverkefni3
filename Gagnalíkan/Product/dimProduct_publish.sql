...
IF EXISTS (
    SELECT 1
    FROM [H10].[dimProduct_stg]
    WHERE [rowBatchKey] = @BatchId
          AND ([productName] IS NULL OR [category] IS NULL OR [cost] IS NULL OR [price] IS NULL)
    -- IF NULLS are found in the staging table, we add the details to the ErrorLog table instead and then continue to the next batch
)
BEGIN
...
SELECT 
    ...
    CASE
        ...
        WHEN src.[cost] IS NULL THEN 'cost'
        WHEN src.[cost] < 0 THEN 'cost'
        WHEN src.[price] IS NULL THEN 'price'
        WHEN src.[price] < 0 THEN 'price'
    END AS ColumnName,
    CASE
